...
static double distance_to_zoid(double *period,
    ...
    for (int dim = 0; dim < 3; dim++) {
        ...
        if (print) {
            std::cout << "dim: " << dim << " lo: " << zoid_lo[dim] << " hi: " << zoid_hi[dim] << " pos: " << pos[dim]
                      << " diff: " << diff << std::endl;
        }
    }
...
#pragma cilk grainsize 512
  ...
  void lammps_fuse_force_compute_lammps_bins_split() {
      ...
      std::cout << "num neighbors atomics within cutoff: " << num_atomics_within_cutoff
                << " num no atomics within cutoff: " << num_no_atomics_within_cutoff << " num same: "
                << num_atomics_same_bin << " num diff: " << num_atomics_diff_bin << " num neighbors atomics: "
                << num_neighbors_atomics << " num neigh no atomics: " << num_neighbors_no_atomics << std::endl;
  }
...
//        }
  ...
  void lammps_fuse_force_compute() {
      ...
      for (int dep = 0; dep < NUM_DEPS_BINS; dep++) {
          ...
          cilk_for(int
          d = 0;
          d < partitions_at_dep.size();
          d++) {
              auto &partition = partitions_at_dep[d];

              // for (int dep_level2 = 0; dep_level2 < dep + 1; dep_level2++) {
              for (int dep_level2 = 0; dep_level2 < atom->num_deps_level2[partition[0]][partition[1]][partition[2]];
                   dep_level2++) {
                  // auto& partitions_at_dep_level2 = atom->dep_to_partitions_level2[dep_level2];
                  auto &partitions_at_dep_level2 =
                      atom->dep_to_partitions_level2[partition[0]][partition[1]][partition[2]][dep_level2];

                  cilk_for(int
                  d2 = 0;
                  d2 < partitions_at_dep_level2.size();
                  d2++) {
                      auto &partition_level2 = partitions_at_dep_level2[d2];
                      auto &bins =
                          atom->partition_to_bins_level2[partition[0]][partition[1]][partition[2]][partition_level2[0]][partition_level2[1]][partition_level2[2]];
                      ...
                      for (int b = 0; b < bins.size(); b++) {
                          ...
                          for (int idx = 0; idx < idxs.size(); idx++) {
                              ...
                              for (int j = 0; j < lst_bonds.size(); j++) {
                                  ...
                                  if (rlogarg < 0.1) {
                                      std::cout << "dep: " << dep << " dep2: " << dep_level2 << " partition level 1: "
                                                << m[partition[0]]
                                                << " " << m[partition[1]] << " " << m[partition[2]]
                                                << " partition level 2: " << m[partition_level2[0]] << " "
                                                << m[partition_level2[1]]
                                                << " " << m[partition_level2[2]] << std::endl;
...
#pragma cilk grainsize 2048
  ...
  void initial_integrate_stencil_md_segments(queue_info &zoid,
                                             int timestep,
                                             Atom *curr,
                                             Atom *next,
...
#pragma cilk grainsize 2048
  ...
  void initial_integrate_stencil_md_affinity(queue_info &zoid,
                                             int timestep,
                                             Atom *curr,
                                             Atom *next,
                                             int *atom_idx_mapping) {
...
#pragma cilk grainsize 1
  ...
  template<bool curr_dt>
  void fuse_initial_integrate_stencil_md(queue_info &zoid,
                                         int timestep,
                                         Atom *curr,
                                         Atom *next,
                                         int *atom_idx_mapping) {
  ...
  void fuse_force_computation_reduce(queue_info &zoid,
                                     int timestep,
                                     Atom *next,
                                     Neighbor *neigh_next,
                                     Force *next_force,
...
//                                return;
  ...
  void stencil_md_fuse_force_computation_atomics(queue_info &zoid,
                                                 int timestep,
                                                 Atom *next,
                                                 Neighbor *neigh_next,
                                                 Force *next_force,
...
#pragma cilk grainsize 512
  ...
  void stencil_md_fuse_force_computation_atomics_affinity(queue_info &zoid,
                                                          int timestep,
                                                          Atom *next,
                                                          Neighbor *neigh_next,
                                                          Force *next_force,
...
#pragma cilk grainsize 1
  ...
  template<bool curr_dt>
  void fuse_force_computation_atomics(queue_info &zoid,
                                      int timestep,
                                      Atom *next,
                                      Neighbor *neigh_next,
                                      Force *next_force,
...
//                                return;
  ...
  template<bool curr_dt>
  void fuse_force_computation(queue_info &zoid,
                              int timestep,
                              Atom *next,
                              Neighbor *neigh_next,
                              Force *next_force,
...
#pragma cilk grainsize 1
  ...
  inline void fuse_post_force_final_integrate_stencil_md_affinity_double_buffering(queue_info &zoid,
                                                                                   int timestep,
                                                                                   Atom *next,
                                                                                   Modify *modify_) {
      auto *_noalias v = zoid.v_stencil_md[timestep % 1].data();
...
#pragma cilk grainsize 1
  ...
  /* Start double buffering code */
  void stencil_md_initial_integrate_affinity_double_buffering_start_end(queue_info &zoid,
                                                                        int timestep,
                                                                        Atom *atom_,
                                                                        const std::vector<int> &space_cut_idxs) {
...
#pragma cilk grainsize 1
  ...
  inline void fuse_post_force_final_integrate_stencil_md_start_end(queue_info &zoid,
                                                                   int timestep,
                                                                   Atom *next,
                                                                   Modify *modify_,
                                                                   std::vector<int> &space_cut_idxs) {
...
#pragma cilk grainsize 1
              ...
              if (zoid.num % comm->nprocs == comm->me) {
                  ...
                  for (int i = 0; i < send_to.size(); i++) {
                      for (int t = 0; t < NUM_TIMESTEPS_IN_PARALLEL + 1; t++) {
                          ...
                          Atom *atom_ =
                              curr_dt ? lmp->atom_stencil_md[zoid.num][t] : lmp->atom_stencil_md[zoid.num][
                          ...
                          for (int k = 0; k < num_segments; k++) {
                              ...
                              for (int h = 0; h < segment_size; h++) {
                                  ...
                                  assert(std::find(send_force_idxs.begin(), send_force_idxs.end(), double_buffering_idx)
                                             == send_force_idxs.end());
                          ...
                          zoid.send_force_idxs_double_buffering[t][i] =
                              std::vector<int>(send_force_idxs.begin(), send_force_idxs.end());
                      }
                  }
              }
          }
      ...
      for (int dep = 0; dep < NUM_DEPS; dep++) {
          for (int j = 0; j < queue[dep].size(); j++) {
              ...
              if (zoid_num % comm->nprocs == comm->me) {
                  auto &recv_from =
                      curr_dt ? lmp->recv_from_neighbors[zoid.num] : lmp->recv_from_neighbors_next_dt[zoid.num];
                  if (recv_from.size() == 0) {
                      continue;
                  }
                  ...
                  for (int i = 0; i < recv_from.size(); i++) {
                      ...
                      for (int t = 0; t < NUM_TIMESTEPS_IN_PARALLEL + 1; t++) {
                          ...
                          for (int k = 0; k < nrecv_force; k++) {
                              ...
                              assert(std::find(recv_force_idxs.begin(), recv_force_idxs.end(), double_buffering_idx)
                                         == recv_force_idxs.end());
  ...
  void CONSTRUCT_RECV_POS_DOUBLE_BUFFERING() {
      ...
      for (int dep = 0; dep < NUM_DEPS; dep++) {
          for (int j = 0; j < my_queue[dep].size(); j++) {
              ...
              if (zoid.num % comm->nprocs == comm->me) {
                  auto &recv_from =
                      curr_dt ? lmp->recv_from_neighbors[zoid_num] : lmp->recv_from_neighbors_next_dt[zoid_num];
  ...
  void TEST_FORCE_AGAINST_LAMMPS_DOUBLE_BUFFERING(bool curr_dt,
                                                  int timestep,
                                                  queue_info &zoid,
                                                  Atom *atom_,
                                                  double **test_f) {
      ...
      for (int k = 0; k < atom_->nlocal; k++) {
          ...
          for (int dim = 0; dim < 3; dim++) {
              ...
              if (fabs(my_force - test_f[timestep][tag * 3 + dim]) > 1e-6) {
                  ...
                  std::cout << "idx: " << k << " out of: " << atom_->nlocal << " double buffering idx: " << test_idx
                            << std::endl;
                  std::cout << "Dim: " << dim << " Zoid: " << zoid.num << " timestep: " << timestep << " tag: " << tag
                            << std::endl;
                  std::cout << "what I have: " << my_f[0] + my_eval_f[0] << " " << my_f[1] + my_eval_f[1] << " "
                            << my_f[2] + my_eval_f[2] << std::endl;
  ...
  void TEST_POS_AGAINST_LAMMPS_DOUBLE_BUFFERING(bool curr_dt,
                                                int timestep,
                                                queue_info &zoid,
                                                Atom *atom_,
                                                double **test_x) {
  ...
  void TEST_VEL_AGAINST_LAMMPS_DOUBLE_BUFFERING(bool curr_dt,
                                                int timestep,
                                                queue_info &zoid,
                                                Atom *atom_,
                                                double **test_v) {
      ...
      for (int k = 0; k < atom_->nlocal; k++) {
          ...
          for (int dim = 0; dim < 3; dim++) {
              if (fabs(my_v[dim] - test_v[timestep][tag * 3 + dim]) > 1e-6) {
                  ...
                  std::cout << "idx: " << k << " out of: " << atom_->nlocal << " atom: " << atom_ << " real idx: "
                            << test_idx
                            << std::endl;
                  std::cout << "Dim: " << dim << " Zoid: " << zoid.num << " timestep: " << timestep << " tag: " << tag
                            << std::endl;
  ...
  void TEST_AGAINST_LAMMPS_FORCE_DOUBLE_BUFFERING_SETUP(double *test_f, queue_info &zoid, int timestep) {
      ...
      for (int i = 0; i < local_idxs.size(); i++) {
          ...
          bool all_close = (fabs(test_f[tag * 3] - my_x) < 5e-5) && (fabs(test_f[tag * 3 + 1] - my_y) < 5e-5)
              && (fabs(test_f[tag * 3 + 2] - my_z) < 5e-5);

          if (!all_close) {
              std::cout << RED << "ERROR ON FORCE zoid: " << zoid.num << " idx: " << idx << " tag: " << tag
                        << " what I have: " << my_x << " " << my_y << " " << my_z
                        << " what lammps has: " << test_f[tag * 3] << " " << test_f[tag * 3 + 1] << " "
                        << test_f[tag * 3 + 2]
                        << " diff: "
                        << fabs(test_f[tag * 3] - my_x) << " " << fabs(test_f[tag * 3 + 1] - my_y) << " "
                        << fabs(test_f[tag * 3 + 2] - my_z)
  ...
  void TEST_FORCE_AGAINST_LAMMPS_DOUBLE_BUFFERING_SPACE_CUT(bool curr_dt,
                                                            int timestep,
                                                            queue_info &zoid,
                                                            Atom *atom_,
                                                            double **test_f,
  ...
  void TEST_POS_AGAINST_LAMMPS_DOUBLE_BUFFERING_SPACE_CUT(bool curr_dt,
                                                          int timestep,
                                                          queue_info &zoid,
                                                          Atom *atom_,
                                                          double **test_x,
  ...
  void TEST_VEL_AGAINST_LAMMPS_DOUBLE_BUFFERING_SPACE_CUT(bool curr_dt,
                                                          int timestep,
                                                          queue_info &zoid,
                                                          Atom *atom_,
                                                          double **test_v,
  ...
  void INIT_ZOID_MANY_CUTS() {
      ...
      for (int dim = 0; dim < domain->dimension; dim++) {
          ...
          double narrow_base_width =
              ((width / num_cuts_in_dimension) - 2 * NUM_TIMESTEPS_IN_PARALLEL * ALLEGRO_SLOPE) / 2 + 0.1;
          double wide_base_width = (width - num_cuts_in_dimension * narrow_base_width) / num_cuts_in_dimension;

          std::cout << "dim: " << dim << " narrow base width: " << narrow_base_width << " wide base width: "
                    << wide_base_width
                    << " total: " << num_cuts_in_dimension * (narrow_base_width + wide_base_width) << " width: "
                    << width
                    << std::endl;
      ...
      if (comm->me == 0) {
          for (int dim = 0; dim < domain->dimension; dim++) {
              std::cout << BOLDCYAN << "dim: " << dim << " Lo: " << domain->boxlo[dim] << " hi: " << domain->boxhi[dim]
                        << RESET_COLOR << std::endl;
              std::vector<double> bounds;
              if (dim == 0) {
                  bounds = bounds_x;
  ...
  void INIT_ZOIDS_NUMBERING() {
      ...
      if (TRY_ONLY_MAX_FLOW) {
          for (int dep = 1; dep < NUM_DEPS; dep++) {
              ...
              for (int proc = 0; proc < comm->nprocs; proc++) {
                  if (comm->me == 0) {
                      std::cout << "dep: " << dep << " proc: " << proc << " has num zoids: "
                                << proc_to_zoids[proc].size()
                                << std::endl;
                  }
              }
      ...
      for (int dep = 1; dep < NUM_DEPS; dep++) {
          ...
          for (int proc = 0; proc < comm->nprocs; proc++) {
              if (comm->me == 0) {
                  std::cout << "dep: " << dep << " proc: " << proc << " has num zoids: " << proc_to_zoids[proc].size()
                            << std::endl;
              }
          }
      ...
      for (int dep = 0; dep < NUM_DEPS; dep++) {
          for (int j = 0; j < queues_many_cuts[dep].size(); j++) {
              ...
              if (zoid.num % comm->nprocs == comm->me) {
                  ...
                  zoid.send_force_idxs_double_buffering_flattened_pipelined =
                      new std::vector<int> *[NUM_PIPELINE_STAGES];
                  zoid.send_pos_idxs_double_buffering_flattened_pipelined =
                      new std::vector<int> **[NUM_PIPELINE_STAGES];
                  zoid.send_vel_idxs_double_buffering_flattened_pipelined = new std::vector<int> *[NUM_PIPELINE_STAGES];

                  zoid.recv_force_idxs_double_buffering_flattened_pipelined =
                      new std::vector<int> *[NUM_PIPELINE_STAGES];
                  zoid.recv_pos_idxs_double_buffering_flattened_pipelined =
                      new std::vector<int> **[NUM_PIPELINE_STAGES];
                  ...
                  for (int p = 0; p < NUM_PIPELINE_STAGES; p++) {
                      zoid.send_force_idxs_double_buffering_flattened_pipelined[p] =
                          new std::vector<int>[MAX_NEIGHBORS];
                      zoid.send_pos_idxs_double_buffering_flattened_pipelined[p] =
                          new std::vector<int> *[MAX_NEIGHBORS];
                      zoid.send_vel_idxs_double_buffering_flattened_pipelined[p] = new std::vector<int>[MAX_NEIGHBORS];

                      zoid.recv_force_idxs_double_buffering_flattened_pipelined[p] =
                          new std::vector<int>[MAX_NEIGHBORS];
                      zoid.recv_pos_idxs_double_buffering_flattened_pipelined[p] =
                          new std::vector<int> *[MAX_NEIGHBORS];
                      zoid.recv_vel_idxs_double_buffering_flattened_pipelined[p] = new std::vector<int>[MAX_NEIGHBORS];

                      for (int k = 0; k < DOUBLE_BUFFERING; k++) {
                          zoid.send_pos_idxs_double_buffering_flattened_pipelined[p][k] =
                              new std::vector<int>[MAX_NEIGHBORS];
                          zoid.recv_pos_idxs_double_buffering_flattened_pipelined[p][k] =
                              new std::vector<int>[MAX_NEIGHBORS];
                      }
                  }
          ...
          for (int i = 0; i < NUM_ZOIDS_MANY_CUTS; i++) {
              ...
              for (int j = 0; j < recv_neighbors.size(); j++) {
                  if (recv_neighbors[j] % comm->nprocs != zoid.num % comm->nprocs) {
                      ...
                      int recv_zoid_dep =
                          (recv_zoid.where[0] % 2 == 0) + (recv_zoid.where[1] % 2 == 0) + (recv_zoid.where[2] % 2 == 0);
                      ...
                      if (in_zoid || borders_zoid) {
                          if (tags_in_zoid.find(idx) == tags_in_zoid.end()) {
                              ...
                              if (in_zoid) {
                                  ...
                                  zoid.x_stencil_md[0].push_back({new_pos_borders[0], new_pos_borders[1],
                                                                  new_pos_borders[2]});
                                  zoid.x_stencil_md[1].push_back({new_pos_borders[0], new_pos_borders[1],
                                                                  new_pos_borders[2]});
                              }
                              zoid.v_stencil_md[0].push_back({all_vel[idx * 3 + 0], all_vel[idx * 3 + 1],
                                                              all_vel[idx * 3 + 2]});
  ...
  void SORT_LOCAL_ATOMS_ZOID_MANY_CUTS() {
          ...
          j++) {
              ...
              if (zoid_num % comm->nprocs == comm->me) {
                  ...
                  for (int idx = 0; idx < zoid.x_stencil_md[0].size(); idx++) {
                      double pos[3] =
                          {zoid.x_stencil_md[0][idx].x, zoid.x_stencil_md[0][idx].y, zoid.x_stencil_md[0][idx].z};
                  ...
                  if (TRY_ORIGINAL) {
                          ...
                          for (int t2 = 0; t2 < NUM_TIMESTEPS_IN_PARALLEL + 1; t2++) {
                              ...
                              if (in_zoid) {
                                  ...
                                  for (int r = 0; r < recv_neighbors.size(); r++) {
                                      ...
                                      double neigh_zoid_borders_lo[3] =
                                          {recv_zoid.lo[t2][0] - ALLEGRO_SLOPE, recv_zoid.lo[t2][1] - ALLEGRO_SLOPE,
                                           recv_zoid.lo[t2][2] - ALLEGRO_SLOPE};
                                      double neigh_zoid_borders_hi[3] =
                                          {recv_zoid.hi[t2][0] + ALLEGRO_SLOPE, recv_zoid.hi[t2][1] + ALLEGRO_SLOPE,
                                           recv_zoid.hi[t2][2] + ALLEGRO_SLOPE};

                                      bool in_neighbor_zoid =
                                          in_zoid_helper(pos, neigh_zoid_borders_lo, neigh_zoid_borders_hi);
                                      if (in_neighbor_zoid) {
                      ...
                      for (int idx = 0; idx < num_elems; idx++) {
                          ...
                          for (int t2 = 0; t2 < NUM_TIMESTEPS_IN_PARALLEL + 1; t2++) {
                              bool
                                  in_zoid = in_zoid_helper(pos, zoid_next_dt.lo[t2].data(), zoid_next_dt.hi[t2].data());
                              if (in_zoid) {
                                  auto &recv_neighbors = recv_from_neighbors_many_cuts_next_dt[zoid.num];
                                  for (int r = 0; r < recv_neighbors.size(); r++) {
                                      int recv_zoid_num = recv_neighbors[r];
                      ...
                      permutation = sort_permutation(zoid.tag_stencil_md[0],
                                                     [&](const tagint &tag_a, const tagint &tag_b) {
                                                       ...
                                                       int last_timestep_local_a =
                                                           timesteps_local_a[timesteps_local_a.size() - 1];
                                                       int last_timestep_local_b =
                                                           timesteps_local_b[timesteps_local_b.size() - 1];
      ...
      dep++) {
          cilk_for(int
          j = 0;
          j < queues_many_cuts_next_dt[dep].size();
          j++) {
              queue_info &zoid = queues_many_cuts_next_dt[dep][j];
              int zoid_num = zoid.num;
              if (zoid_num % comm->nprocs != comm->me) {
          ...
          for (int i = 0; i < local_idxs.size(); i++) {
              ...
              for (auto &neigh_tag : neigh_set) {
                  if (!zoid_tag_to_idx.count(neigh_tag)) {
                      std::cout << "zoid: " << zoid.num << " timestep: " << t << " tag: " << tag << " neigh tag: "
                                << neigh_tag
                                << std::endl;
                      assert(false);
                  }
                          ...
                          if (tag_to_timestep_even.count(tag)) {
                              if (tag_to_timestep_even.at(tag) != t) {
                                  ...
                                  std::cout << std::setprecision(20)
                                            ...
                                            << "EVEN tag: " << tag << " timestep: " << t
                                            << " overlapping recv pos timestep: "
                                            << tag_to_timestep_even.at(tag)
                                            << " pos: " << zoid.x_stencil_md[0][i].x << " " << zoid.x_stencil_md[0][i].y
                                            << " "
                                            << zoid.x_stencil_md[0][i].z
          ...
          for (int j = 0; j < queues[dep].size(); j++) {
              ...
              for (int t = 0; t < NUM_TIMESTEPS_IN_PARALLEL + 1; t++) {
                  ...
                  for (int i = 0; i < send_to_neighbors.size(); i++) {
                      ...
                      r.emplace_back();
                      MPI_Isend(&zoid_send_data_sizes[zoid.num][t][i],
                                1,
                                MPI_INT,
                      ...
                      if (send_force_idxs.size() > 0) {
                          ...
                          // std::vector<int> send_pos_tags;
                          // send_pos_tags.reserve(size);
                          MPI_Isend(zoid_send_data[zoid.num][t][i].data(),
                                    send_force_idxs.size(),
                                    MPI_INT,
                                    send_zoid_num % comm->nprocs,
                                    mpi_tag,
                                    world,
                              ...
                              if (tag_to_timestep.count(recv_buf[k])) {
                                  if (tag_to_timestep.at(recv_buf[k]) != t) {
                                      ...
                                      std::cout << std::setprecision(std::numeric_limits<double>::digits10 + 1)
                                                << "tag: " << recv_buf[k] << " timestep: " << t
                                                << " overlapping recv force timestep: "
                                                << tag_to_timestep.at(recv_buf[k])
                                                << " pos: " << zoid.x_stencil_md[0][tag_idx].x << " "
                                                << zoid.x_stencil_md[0][tag_idx].y
                                                << " " << zoid.x_stencil_md[0][tag_idx].z
                                                << " lo: " << zoid.lo[t][0] << " " << zoid.lo[t][1] << " "
                                                << zoid.lo[t][2]
                                                << " hi: " << zoid.hi[t][0] << " " << zoid.hi[t][1] << " "
                                                << zoid.hi[t][2]
                  ...
                  for (int i = 0; i < recv_from_neighbors.size(); i++) {
                      ...
                      for (int k = 0; k < recv_idxs.size(); k++) {
                          ...
                          if (all_idxs.find(idx) != all_idxs.end()) {
                              ...
                              if (map_timestep != t) {
                                  std::cout << "RECV FORCE zoid: " << zoid.num << " timestep: " << t << " IDX: " << idx
                                            << " curr recv from: " << recv_from_neighbors[i] << " timestep: " << t
                                            << " already there: " << idx_to_zoid_timestep[idx].first << " timestep: "
                                            << idx_to_zoid_timestep[idx].second
              ...
              for (int t = 0; t < NUM_TIMESTEPS_IN_PARALLEL + 1; t++) {
                  for (int i = 0; i < recv_from_neighbors.size(); i++) {
                      ...
                      for (int k = 0; k < recv_idxs.size(); k++) {
                          int idx = recv_idxs[k];
                          if (all_idxs.find(idx) != all_idxs.end()) {
                              std::cout << "RECV VEL: " << zoid.num << " IDX: " << idx << " curr recv from: "
                                        << recv_from_neighbors[i] << " timestep: " << t
                                        << " already there: " << idx_to_zoid_timestep[idx].first << " timestep: "
                                        << idx_to_zoid_timestep[idx].second
                                        << " tag: " << zoid.tag_stencil_md[0][idx]
                                        << std::endl;
  ...
  void CONSTRUCT_NEW_RECV_FORCE_IDXS_FLATTENED() {
      ...
      for (int dep = 0; dep < NUM_DEPS; dep++) {
          for (int j = 0; j < my_queues[dep].size(); j++) {
              ...
              auto &recv_neighbors =
                  curr_dt ? recv_from_neighbors_many_cuts[zoid.num] : recv_from_neighbors_many_cuts_next_dt[zoid.num];
              for (int i = 0; i < recv_neighbors.size(); i++) {
                  int recv_zoid_num = recv_neighbors[i];
                  ...
                  zoid_send_data[zoid.num][i].insert(zoid_send_data[zoid.num][i].end(),
                                                     permutation.begin(),
                                                     permutation.end());
                  int mpi_tag = get_mpi_tag_many_cuts(recv_zoid_num, zoid.num);
                  r.emplace_back();
                  MPI_Isend(zoid_send_data[zoid.num][i].data(),
                            permutation.size(),
                            MPI_INT,
      ...
      for (int dep = 0; dep < NUM_DEPS; dep++) {
          for (int j = 0; j < my_queues[dep].size(); j++) {
              ...
              for (int i = 0; i < recv_neighbors.size(); i++) {
                  ...
                  zoid_send_data0[zoid.num][i].insert(zoid_send_data0[zoid.num][i].end(),
                                                      permutation0.begin(),
                                                      permutation0.end());
                  int mpi_tag = get_mpi_tag_many_cuts(recv_zoid_num, zoid.num);
                  r.emplace_back();
                  ...
                  zoid_send_data1[zoid.num][i].insert(zoid_send_data1[zoid.num][i].end(),
                                                      permutation1.begin(),
                                                      permutation1.end());
                  r.emplace_back();
      ...
      for (int dep = 0; dep < NUM_DEPS; dep++) {
          for (int j = 0; j < my_queues[dep].size(); j++) {
              ...
              for (int i = 0; i < recv_neighbors.size(); i++) {
                  ...
                  std::stringstream o;
                  o << BOLDGREEN << "curr_dt ? " << curr_dt << " zoid: " << zoid.num << " RECV FROM: "
                    << recv_neighbors[i]
  ...
  void GET_SEND_STATISTICS() {
      ...
      for (int dep = 0; dep < NUM_DEPS; dep++) {
          for (int j = 0; j < queues[dep].size(); j++) {
              ...
              auto &send_neighbors =
                  curr_dt ? send_to_neighbors_many_cuts[zoid_num] : send_to_neighbors_many_cuts_next_dt[zoid_num];
              ...
              for (int i = 0; i < send_neighbors.size(); i++) {
                  ...
                  auto &send_zoid = curr_dt ? zoid_num_to_zoid_many_cuts[send_zoid_num]
                                            : zoid_num_to_zoid_many_cuts_next_dt[send_zoid_num];
                  int send_zoid_dep =
                      (send_zoid.where[0] % 2 == 0) + (send_zoid.where[1] % 2 == 0) + (send_zoid.where[2] % 2 == 0);
                  if (send_zoid_num % comm->nprocs != comm->me) {
                      ...
                      if (send_zoid_dep == my_zoid_dep + 1) {
                          ...
                          std::stringstream s1;
                          s1 << "zoid: " << zoid.num << " curr_dt: " << curr_dt << " dep: " << my_zoid_dep << " send: "
                             << send_zoid_dep << " total num doubles: "
                             << (total_send_force + total_send_pos + total_send_vel) * 3
                             << " force: " << total_send_force << " pos: " << total_send_pos << " vel: "
                             << total_send_vel
                             << std::endl;
                          std::cout << s1.str();
                      }
  ...
  void GET_RECV_STATISTICS() {
      ...
      for (int dep = 0; dep < NUM_DEPS; dep++) {
          for (int j = 0; j < queues[dep].size(); j++) {
              ...
              auto &recv_neighbors =
                  curr_dt ? recv_from_neighbors_many_cuts[zoid_num] : recv_from_neighbors_many_cuts_next_dt[zoid_num];
              ...
              for (int i = 0; i < recv_neighbors.size(); i++) {
                  ...
                  int total_recv_pos = zoid.recv_pos_idxs_double_buffering_flattened[0][i].size()
                      + zoid.recv_pos_idxs_double_buffering_flattened[1][i].size();
      ...
      if (comm->me == 0) {
          std::cout << "total recv: " << total_recv_proc << " total recv through MPI: " << total_recv_proc_mpi
                    << " dt: "
                    << NUM_TIMESTEPS_IN_PARALLEL << std::endl;
      }
  }
  ...
  void CONSTRUCT_RECV_PROC_OFFSETS() {
      ...
      for (int proc = 0; proc < comm->nprocs; proc++) {
          for (int send_dep = 0; send_dep < NUM_DEPS - 1; send_dep++) {
              ...
              int total_doubles_recv_from_proc =
                  DEBUG_SEND_RECV_DATA ? total_nrecv_from_proc * (3 + 1) : total_nrecv_from_proc * 3;
  ...
  void CONSTRUCT_SEND_PROC_OFFSETS() {
      ...
      for (int proc = 0; proc < comm->nprocs; proc++) {
          for (int send_dep = 0; send_dep < NUM_DEPS - 1; send_dep++) {
              ...
              int total_doubles_send_to_proc =
                  DEBUG_SEND_RECV_DATA ? total_nsend_to_proc * (3 + 1) : total_nsend_to_proc * 3;
      ...
      for (int zoid_num = 0; zoid_num < NUM_ZOIDS_MANY_CUTS; zoid_num++) {
          ...
          if (num_send_neighbors > 0) {
              if (curr_dt) {
                  ...
                  std::fill(send_zoid_to_zoid_sizes_setup[zoid_num].begin(),
                            send_zoid_to_zoid_sizes_setup[zoid_num].end(),
                            0);
              } else {
                  send_zoid_to_zoid_sizes_next_dt[zoid_num].resize(num_send_neighbors);
                  std::fill(send_zoid_to_zoid_sizes_next_dt[zoid_num].begin(),
                            send_zoid_to_zoid_sizes_next_dt[zoid_num].end(),
                            0);
              }
          }
      }
      ...
      for (int send_dep = 0; send_dep < NUM_DEPS - 1; send_dep++) {
          for (int j = 0; j < queues[send_dep].size(); j++) {
              ...
              for (int i = 0; i < send_neighbors.size(); i++) {
                  ...
                  if (total_doubles_send_to_zoid
                      > nsend_buf_send_zoid_to_zoid[DEFAULT_PIPELINE_STAGE][send_zoid_num][i]) {
                      GROW_SEND_ZOID_TO_ZOID_MANY_CUTS(send_zoid.num,
                                                       i,
                                                       total_doubles_send_to_zoid,
                  ...
                  if (curr_dt) {
                      ...
                      if (neigh % comm->nprocs != comm->me && total_doubles_send_to_zoid > 0) {
                          send_to_neighbors_not_my_proc_idxs_next_dt[send_zoid.num].push_back(
                              send_neighbor_not_in_proc_idx++);
                      } else {
              ...
              for (int j = 0; j < queues[send_dep].size(); j++) {
                  ...
                  for (int i = 0; i < send_neighbors.size(); i++) {
                      ...
                      for (int t = start_t[p]; t < end_t[p]; t++) {
                          ...
                          for (auto &idx : send_zoid.send_pos_idxs_double_buffering[t][i]) {
                              send_zoid.send_pos_idxs_double_buffering_flattened_pipelined[p][t
                                  % DOUBLE_BUFFERING][i].push_back(idx);
                          }
                      }
                  }
              }
      ...
      for (int p = 0; p < NUM_PIPELINE_STAGES; p++) {
          ...
          for (int zoid_num = 0; zoid_num < NUM_ZOIDS_MANY_CUTS; zoid_num++) {
              ...
              for (int i = 0; i < recv_neighbors.size(); i++) {
                  ...
                  int total_doubles_recv_from_zoid =
                      DEBUG_SEND_RECV_DATA ? nrecv_from_zoid * (3 + 1) : nrecv_from_zoid * 3;
                  ...
                  for (int t = start_t[p]; t < end_t[p]; t++) {
                      ...
                      for (auto &idx : zoid.recv_pos_idxs_double_buffering[t][i]) {
                          zoid.recv_pos_idxs_double_buffering_flattened_pipelined[p][t % DOUBLE_BUFFERING][i].push_back(
                              idx);
                      }
                  }
              }
      ...
      for (int i = 0; i < send_neighbors.size(); i++) {
          ...
          if (buf_idx > 0 && (send_zoid_num % comm->nprocs != comm->me)) {
              ...
              if (USE_STREAMS) {
                  ...
                  int recv_stream_idx =
                      curr_dt ? zoid_to_stream_num[send_zoid_num] : zoid_to_stream_num_next_dt[send_zoid_num];
  ...
  template<bool curr_dt>
  void PACK_AND_SEND_DATA_ZOID_TO_ZOID_REVISED(queue_info &zoid,
                                               int dep,
                                               int start_t,
                                               int end_t,
                                               std::vector<MPI_Request> &r) {
  ...
  template<bool curr_dt>
  void SEND_DATA_ZOID_TO_ZOID_TO_DEP_REVISED(queue_info &zoid,
                                             int send_dep,
                                             int start_t,
                                             int end_t,
                                             std::vector<MPI_Request> &r) {
  ...
  template<bool curr_dt>
  void SEND_DATA_ZOID_TO_ZOID_TO_DEP_REVISED_PIPELINED(queue_info &zoid,
                                                       int send_dep,
                                                       int start_t,
                                                       int end_t,
                                                       int pipeline_stage,
  ...
  void PACK_AND_SEND_DATA_ZOID_TO_ZOID_PIPELINED(queue_info &zoid, int dep,
      ...
      cilk_for(int i = 0; i < send_neighbors.size(); i++) {
          ...
          int buf_idx = PACK_DATA_MANY_CUTS_HELPER_PIPELINED<curr_dt>(zoid,
                                                            buf,
                                                            i,
                                                            send_zoid_num,
                                                            start_t,
  ...
  void RECEIVE_DATA_ZOID_TO_ZOID_WAITANY(int dep, int zoid_num, std::vector<MPI_Request> &r) {
      ...
      for (int i = 0; i < recv_neighbors.size(); i++) {
          ...
          if (total_doubles_recv_from_zoid > 0) {
              ...
              if (USE_STREAMS) {
                  int send_stream_idx =
                      curr_dt ? zoid_to_stream_num[recv_zoid_num] : zoid_to_stream_num_next_dt[recv_zoid_num];
  ...
  template<bool curr_dt>
  void RECEIVE_DATA_ZOID_TO_ZOID_WAITANY_PIPELINED(int dep,
                                                   int zoid_num,
                                                   int pipeline_stage,
                                                   std::vector<MPI_Request> &r) {
      auto &queues = curr_dt ? queues_many_cuts : queues_many_cuts_next_dt;
          ...
          for (int i = 0; i < recv_pos_idxs.size(); i++) {
              ...
              if (target_tag != zoid.tag_stencil_md[0][idx]) {
                  std::cout << "POS me: " << comm->me << " my zoid: " << recv_zoid.num
                            ...
                            << " recv proc: " << recv_zoid_num % comm->nprocs << " tag I got: " << target_tag
                            << " tag I want: "
                            << recv_zoid.tag_stencil_md[0][idx]
                            << std::endl;
              }
...
#pragma cilk grainsize 2048
  ...
  int UNPACK_DATA_MANY_CUTS_ZOID_SELF_ONLY_PIPELINED(queue_info &zoid, int start_t, int end_t, int pipeline_stage) {
      ...
      for (int i = 0; i < recv_neighbors.size(); i++) {
          ...
          int find_idx = std::distance(send_neighbors.begin(), find_it);
          assert(send_neighbors[find_idx] == zoid_num);
          UNPACK_DATA_MANY_CUTS_HELPER_SELF_PIPELINED<curr_dt>(zoid,
                                                               i,
                                                               recv_zoid_num,
                                                               find_idx,
                                                               start_t,
  ...
  template<bool curr_dt>
  void UNPACK_POS_VEL_MANY_CUTS_ZOID_PIPELINED(queue_info &zoid,
                                               int recv_zoid_num,
                                               int start_t,
                                               int end_t,
                                               int pipeline_stage) {
      ...
      UNPACK_POS_VEL_MANY_CUTS_HELPER_PIPELINED<curr_dt>(zoid,
                                                         buf,
                                                         find_idx,
                                                         recv_zoid_num,
                                                         start_t,
                      ...
                      for (int k = 0; k < recv_force_idxs.size(); k++) {
                          ...
                          if (target_tag != recv_zoid.tag_stencil_md[0][idx]) {
                              std::cout << "FORCE TAG WRONG me: " << comm->me << " my zoid: " << recv_zoid.num
                                        ...
                                        << " recv from proc: " << proc << " tag I got: " << target_tag
                                        << " tag I want: "
                                        << recv_zoid.tag_stencil_md[0][idx]
                      ...
                      for (int k = 0; k < recv_pos_idxs.size(); k++) {
                          ...
                          if (target_tag != recv_zoid.tag_stencil_md[0][idx]) {
                              std::cout << "POS me: " << comm->me << " my zoid: " << recv_zoid.num
                                        ...
                                        << " recv proc: " << proc << " tag I got: " << target_tag << " tag I want: "
                                        << recv_zoid.tag_stencil_md[0][idx]
                                        << " offset: " << offset
                                        << std::endl;
                          }
