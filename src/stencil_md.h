...
public:
    ...
    static constexpr int NUM_CUTS_X = 12;
    static constexpr int NUM_CUTS_Y = 12;
    ...
    void CONSTRUCT_RECV_FORCE_IDXS_ZOID_MANY_CUTS() {
        ...
        MPI_Comm force_idx_comm = MPI_COMM_NULL;
        MPI_Comm_dup(world, &force_idx_comm);
        ...
        std::vector<std::vector<int>> send_payloads;
        send_payloads.reserve(NUM_ZOIDS_MANY_CUTS * (NUM_TIMESTEPS_IN_PARALLEL + 1));
        std::vector<SendSizeMsg> size_sends;
        std::vector<SendDataMsg> data_sends;
        std::vector<RecvExpectation> recv_expectations;
        ...
        for (int dep = 0; dep < NUM_DEPS; dep++) {
            for (int j = 0; j < queues[dep].size(); j++) {
                ...
                const auto& send_to_neighbors = curr_dt ? send_to_neighbors_many_cuts[zoid.num]
                                                        : send_to_neighbors_many_cuts_next_dt[zoid.num];
                for (int t = 0; t < NUM_TIMESTEPS_IN_PARALLEL + 1; t++) {
                    for (int i = 0; i < send_to_neighbors.size(); i++) {
                        int send_zoid_num = send_to_neighbors[i];
                        ...
                        send_payloads.emplace_back();
                        auto& payload = send_payloads.back();
                        payload.reserve(send_force_idxs.size());
                        for (int k = 0; k < send_force_idxs.size(); k++) {
                            payload.push_back(zoid.tag_stencil_md[0][send_force_idxs[k]]);
                        }

                        size_sends.push_back({send_zoid_num % comm->nprocs, mpi_tag, static_cast<int>(payload.size())});
                        if (!payload.empty()) {
                            data_sends.push_back({send_zoid_num % comm->nprocs, mpi_tag, static_cast<int>(send_payloads.size() - 1)});
                        }
                    }
                }

                const auto& recv_from_neighbors = curr_dt ? recv_from_neighbors_many_cuts[zoid.num]
                                                          : recv_from_neighbors_many_cuts_next_dt[zoid.num];
                for (int t = 0; t < NUM_TIMESTEPS_IN_PARALLEL + 1; t++) {
                    zoid.recv_force_idxs_double_buffering[t] = new std::vector<int>[recv_from_neighbors.size()];
                    for (int i = 0; i < recv_from_neighbors.size(); i++) {
                        int recv_zoid_num = recv_from_neighbors[i];
                        int mpi_tag = get_mpi_tag_many_cuts(zoid.num, recv_zoid_num);
                        recv_expectations.push_back({&zoid, t, i, recv_zoid_num % comm->nprocs, mpi_tag, 0, {}});
                    }
                }
            }
        }
        ...
        for (int i = 0; i < recv_expectations.size(); i++) {
            auto& exp = recv_expectations[i];
            MPI_Irecv(&exp.size, 1, MPI_INT, exp.src_proc, exp.tag, force_idx_comm, &recv_size_reqs[i]);
        }

        std::vector<MPI_Request> send_size_reqs(size_sends.size(), MPI_REQUEST_NULL);
        for (int i = 0; i < size_sends.size(); i++) {
            auto& msg = size_sends[i];
            MPI_Isend(&msg.size, 1, MPI_INT, msg.dst_proc, msg.tag, force_idx_comm, &send_size_reqs[i]);
        }
        ...
        for (int i = 0; i < recv_expectations.size(); i++) {
            ...
            exp.payload.resize(exp.size);
            MPI_Request req = MPI_REQUEST_NULL;
            MPI_Irecv(exp.payload.data(), exp.size, MPI_INT, exp.src_proc, exp.tag, force_idx_comm, &req);
            recv_data_reqs.push_back(req);
        }
        ...
        for (int i = 0; i < data_sends.size(); i++) {
            auto& msg = data_sends[i];
            auto& payload = send_payloads[msg.payload_idx];
            MPI_Isend(payload.data(), payload.size(), MPI_INT, msg.dst_proc, msg.tag, force_idx_comm, &send_data_reqs[i]);
        }
        ...
        std::vector<std::unordered_map<int, int>> tag_to_idx_by_zoid(NUM_ZOIDS_MANY_CUTS);
        std::vector<std::map<int, int>> tag_to_timestep_by_zoid(NUM_ZOIDS_MANY_CUTS);
        for (int dep = 0; dep < NUM_DEPS; dep++) {
            for (int j = 0; j < queues[dep].size(); j++) {
                auto& zoid = queues[dep][j];
                if (zoid.num % comm->nprocs != comm->me) continue;
                ...
                m.reserve(zoid.tag_stencil_md[0].size());
                for (int i = 0; i < zoid.tag_stencil_md[0].size(); i++) {
                    m[zoid.tag_stencil_md[0][i]] = i;
                }
            }
        }

        for (const auto& exp : recv_expectations) {
            auto& out = exp.zoid->recv_force_idxs_double_buffering[exp.t][exp.neighbor_idx];
            out.reserve(exp.payload.size());

            auto& tag_to_idx = tag_to_idx_by_zoid[exp.zoid->num];
            auto& tag_to_timestep = tag_to_timestep_by_zoid[exp.zoid->num];
            for (int k = 0; k < exp.payload.size(); k++) {
                int tag = exp.payload[k];
                auto it = tag_to_idx.find(tag);
                if (it == tag_to_idx.end()) {
                    std::cerr << "Missing tag in CONSTRUCT_RECV_FORCE_IDXS_ZOID_MANY_CUTS: "
                              << "curr_dt=" << curr_dt << " me=" << comm->me
                              << " zoid=" << exp.zoid->num << " timestep=" << exp.t
                              << " recv_tag=" << tag << std::endl;
                    MPI_Abort(world, 1);
                }

                int idx = it->second;
                out.push_back(idx);
                if (tag_to_timestep.count(tag)) {
                    if (tag_to_timestep.at(tag) != exp.t) {
                        int other_t = tag_to_timestep.at(tag);
                        std::cout << std::setprecision(std::numeric_limits<double>::digits10 + 1)
                                  << "tag: " << tag << " timestep: " << exp.t
                                  << " overlapping recv force timestep: " << other_t
                                  << " pos: " << exp.zoid->x_stencil_md[0][idx].x << " "
                                  << exp.zoid->x_stencil_md[0][idx].y << " " << exp.zoid->x_stencil_md[0][idx].z
                                  << " lo: " << exp.zoid->lo[exp.t][0] << " " << exp.zoid->lo[exp.t][1] << " " << exp.zoid->lo[exp.t][2]
                                  << " hi: " << exp.zoid->hi[exp.t][0] << " " << exp.zoid->hi[exp.t][1] << " " << exp.zoid->hi[exp.t][2]
                                  << " other lo: " << exp.zoid->lo[other_t][0] << " " << exp.zoid->lo[other_t][1] << " " << exp.zoid->lo[other_t][2]
                                  << " other hi: " << exp.zoid->hi[other_t][0] << " " << exp.zoid->hi[other_t][1] << " " << exp.zoid->hi[other_t][2]
                                  << std::endl;
                    }
                    assert(tag_to_timestep.at(tag) == exp.t);
                } else {
                    tag_to_timestep[tag] = exp.t;
                }
            }
        }

        MPI_Comm_free(&force_idx_comm);
    ...
    void CONSTRUCT_RECV_VEL_IDXS_ZOID_MANY_CUTS() {
        MPI_Comm vel_idx_comm = MPI_COMM_NULL;
        MPI_Comm_dup(world, &vel_idx_comm);
            ...
            for (int j = 0; j < queues[dep].size(); j++) {
                ...
                for (int t = 0; t < NUM_TIMESTEPS_IN_PARALLEL + 1; t++) {
                    for (int i = 0; i < send_to_neighbors.size(); i++) {
                        ...
                        send_payloads.emplace_back();
                        auto& payload = send_payloads.back();
                        payload.reserve(send_vel_idxs.size());
                        for (int k = 0; k < send_vel_idxs.size(); k++) {
                            payload.push_back(zoid.tag_stencil_md[0][send_vel_idxs[k]]);
                        }
        ...
        for (int i = 0; i < recv_expectations.size(); i++) {
            auto& exp = recv_expectations[i];
            MPI_Irecv(&exp.size, 1, MPI_INT, exp.src_proc, exp.tag, vel_idx_comm, &recv_size_reqs[i]);
        }

        std::vector<MPI_Request> send_size_reqs(size_sends.size(), MPI_REQUEST_NULL);
        for (int i = 0; i < size_sends.size(); i++) {
            auto& msg = size_sends[i];
            MPI_Isend(&msg.size, 1, MPI_INT, msg.dst_proc, msg.tag, vel_idx_comm, &send_size_reqs[i]);
        }
        ...
        for (int i = 0; i < recv_expectations.size(); i++) {
            ...
            exp.payload.resize(exp.size);
            MPI_Request req = MPI_REQUEST_NULL;
            MPI_Irecv(exp.payload.data(), exp.size, MPI_INT, exp.src_proc, exp.tag, vel_idx_comm, &req);
            recv_data_reqs.push_back(req);
        }
        ...
        for (int i = 0; i < data_sends.size(); i++) {
            auto& msg = data_sends[i];
            auto& payload = send_payloads[msg.payload_idx];
            MPI_Isend(payload.data(), payload.size(), MPI_INT, msg.dst_proc, msg.tag, vel_idx_comm, &send_data_reqs[i]);
        }
        ...
        for (const auto& exp : recv_expectations) {
            auto& out = exp.zoid->recv_vel_idxs_double_buffering[exp.t][exp.neighbor_idx];
            out.reserve(exp.payload.size());
            ...
            for (int k = 0; k < exp.payload.size(); k++) {
                ...
                auto it = tag_to_idx.find(tag);
                if (it == tag_to_idx.end()) {
                    std::cerr << "Missing tag in CONSTRUCT_RECV_VEL_IDXS_ZOID_MANY_CUTS: "
                              << "curr_dt=" << curr_dt << " me=" << comm->me
                              << " zoid=" << exp.zoid->num << " timestep=" << exp.t
                              << " recv_tag=" << tag << std::endl;
                    MPI_Abort(world, 1);
                ...
                if (tag_to_timestep.count(tag)) {
                    if (tag_to_timestep.at(tag) != exp.t) {
                        std::cout << "tag: " << tag << " timestep: " << exp.t
                                  << " overlapping recv vel timestep: " << tag_to_timestep.at(tag)
                                  << std::endl;
                    }
                    assert(tag_to_timestep.at(tag) == exp.t);
                } else {
...
// }
    ...
    void BOND_FENE_FORCE_COMPUTE_ZOID_MANY_CUTS(queue_info& zoid, int dep, int timestep) {
        ...
        int num_chunks_pair = nlocal / PAIR_GRAINSIZE + 1;
