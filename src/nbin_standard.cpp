...
void NBinStandard::setup_bins_stencil_md(int style, Atom* atom_, Domain* domain_, Comm* comm_) {
    // bbox = size of bbox of entire domain
    // bsubbox lo/hi = bounding box of my subdomain extended by comm->cutghost
    // for triclinic:
    //   bbox bounds all 8 corners of tilted box
    ...
    if (triclinic == 0) {
        ...
        lo[1] = domain_->sublo_lamda[1] - cutghost[1];
        lo[2] = domain_->sublo_lamda[2] - cutghost[2];
        hi[0] = domain_->subhi_lamda[0] + cutghost[0];
        hi[1] = domain_->subhi_lamda[1] + cutghost[1];
        hi[2] = domain_->subhi_lamda[2] + cutghost[2];
        domain_->bbox(lo,hi,bsubboxlo,bsubboxhi);
    }
