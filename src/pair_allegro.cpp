...
template<Precision precision>
void PairAllegro<precision>::init_style_stencil_md(Neighbor* neighbor_) {
    if (atom->tag_enable == 0)
        error->all(FLERR,"Pair style Allegro requires atom IDs");
...
void PairAllegro<precision>::coeff(int narg, char **arg) {
  ...
  // std::cout << "atom ntypes? " << ntypes << std::endl;
  ...
  */
  ...
  cutoff = std::stod(lmp->lmp_model_metadata["r_max"]);
  ...
  int n_species = std::stod(lmp->lmp_model_metadata["n_species"]);
  ss << lmp->lmp_model_metadata["type_names"];
...
#pragma omp parallel for reduction(+:nedges)
  ...
  */
  ...
  auto ij2type = ij2type_tensor.accessor<int64_t, 1>();
